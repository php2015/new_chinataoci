...
//                 showMessage($e->getMessage(),'','html','error');
    ...
    private function _setQueryXS($condition,$order) {
        ...
        if (isset($condition['is_book'])) {
            $this->_xs_search->addQueryString('is_book'.':'.$condition['is_book']);
        }
        ...
        if (isset($condition['goods_barcode'])) {
            $this->_xs_search->addQueryString('goods_barcode'.':'.$condition['goods_barcode']);
        }
        ...
        //查询消费者保障服务
        if ($condition['contract'] && is_array($condition['contract'])) {
            foreach ($condition['contract'] as $v) {
                $this->_xs_search->addQueryString('contract_'.$v.':1');
            }
        }
        if (count($order) > 1) {
    ...
    private function _searchXS(){
        ...
        $cates = array();
        foreach ($docs as $doc) {
            foreach (array('goods_id','goods_name','goods_jingle','gc_name','brand_name','brand_id','is_book','attr_id',
                    'store_id','area_id','have_gift','goods_promotion_price','goods_click','goods_salenum','goods_barcode') as $v) {
                $goods_list[$doc->goods_id][$v] = $doc->$v;
                $body = unserialize($doc->main_body);
                if (is_array($body)) {
                    $goods_list[$doc->goods_id] = array_merge($goods_list[$doc->goods_id],$body);
                }
            }
        }
   ...
   public function getTagCategory($keyword = '') {
    ...
    public function indexerSearch($get = array(), $pagesize) {
        ...
        if ($get['barcode'] != '') {
            $condition['goods_barcode'] = $get['barcode'];
        }
        //查询消费者保障服务
        if ($get["search_ci_arr"]) {
            $this->_contract_item = Model('contract')->getContractItemByCache();
            if ($this->_contract_item) {
                $condition["contract"] = array();
                foreach ($get["search_ci_arr"] as $ci_val) {
                    $condition["contract"][] = $ci_val;
                }
            }
        }
        //拼接排序(销量,浏览量,价格)
        ...
        if (in_array($get['key'],array('1','2','3'))) {
            $order = array(str_replace(array('1','2','3'), array('goods_salenum','goods_click','goods_promotion_price'), $get['key'])
