...
$html_title = 'ShopWWI【S3.0】 - 安装向导';

$html_header = <<<EOF
<div class="header">
  <div class="layout">
    <div class="title">
      <h5>ShopWWI【S3.0】电商系统</h5>
      <h2>系统安装向导</h2>
    </div>
    <div class="version">版本: 2016.02.15.0887</div>
  </div>
</div>
...
$html_footer = <<<EOF
<div class="footer">
  <h5>Powered by <font class="blue">Shop</font><font class="orange">WWI</font></h5>
  <h6>版权所有 2013-2016 &copy; <a href="http://www.shopwwi.com" target="_blank">网店运维交流中心</a></h6>
  <h6>软件注册登记编号：运维S3.0</h6>
</div>
EOF;
...
function step3(&$install_error,&$install_recover){
    ...
    if (file_exists('../../mobile/readme/sql/mobile.sql')) {
        $sql .= "\r\n".file_get_contents("../../mobile/readme/sql/mobile.sql");
    }
    ...
    $mysqli->query("INSERT INTO {$db_prefix}album_class (`aclass_id`, `aclass_name`, `store_id`, `aclass_des`, `aclass_sort`, `aclass_cover`, `upload_time`, `is_default`) VALUES(1, '默认相册', 1, '', 255, '', 1387942806, 1)");

    $mysqli->query("UPDATE `{$db_prefix}navigation` SET nav_url=CONCAT('{$auto_site_url}/',nav_url)");
    $mysqli->query("UPDATE `{$db_prefix}cms_navigation` SET navigation_link=CONCAT('{$auto_site_url}/',navigation_link)");
    if ($_POST['demo_data'] == '1'){
        ...
        $mysqli->query("UPDATE `{$db_prefix}groupbuy` SET `store_name` = '{$store_name}'");
        $mysqli->query("UPDATE `{$db_prefix}groupbuy_quota` SET `member_name` = '{$member_name}' ,`store_name` = '{$store_name}';");
