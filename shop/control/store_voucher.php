...
class store_voucherControl extends BaseSellerControl{
    ...
    public function templateaddOp(){
        ...
        if(chksubmit()){
            ...
            if($limit>0 && $price>=$limit) $error.=Language::get('voucher_template_price_error');
            //验证卡密代金券发放数量
            ...
            if ($error){
                ...
                if($rs){
                    //生成卡密代金券
                    if($gettype == 'pwd'){
                        QueueClient::push('build_pwdvoucher', $rs);
                    }
        ...
        if(chksubmit()){
            ...
            if ($error){
                ...
                if($rs){
                    //生成卡密代金券
                    if($gettype == 'pwd'){
                        QueueClient::push('build_pwdvoucher', $t_info['voucher_t_id']);
                    }
    ...
    public function bulidvoucherOp(){
    	...
    	//生成卡密代金券队列
    	QueueClient::push('build_pwdvoucher', $t_id);
