    ...
    public function booth_quota_addOp() {
        if (chksubmit()) {
            ...
            if ($return) {
                ...
                Model('cron')->addCron(array('exetime' => $end_time, 'exeid' => $_SESSION['store_id'], 'type' => 4), true);
                $this->recordSellerLog('购买'.$quantity.'套推荐展位，单价'.intval(C('promotion_booth_price')).'元');
    ...
    public function booth_renewOp() {
        if (chksubmit()) {
            ...
            if ($return) {
                ...
                $end_time = TIMESTAMP + 60 * 60 * 24 * 30 * $quantity;
                Model('cron')->addCron(array('exetime' => $end_time, 'exeid' => $_SESSION['store_id'], 'type' => 4), true);
                $this->recordSellerLog('续费'.$quantity.'套推荐展位，单价'.intval(C('promotion_bundling_price')).'元');
