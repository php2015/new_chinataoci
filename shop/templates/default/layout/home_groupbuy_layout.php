<?php defined('ByShopWWI') or exit('Access Invalid!');?><!doctype html><html lang="zh"><head><meta http-equiv="Content-Type" content="text/html; charset=<?php echo CHARSET;?>"><title><?php echo $output['html_title'];?></title><meta name="keywords" content="<?php echo $output['seo_keywords']; ?>" /><meta name="description" content="<?php echo $output['seo_description']; ?>" /><meta name="author" content="ShopWWI"><meta name="copyright" content="ShopWWI Inc. All Rights Reserved"><meta name="renderer" content="webkit"><meta name="renderer" content="ie-stand"><?php echo html_entity_decode($output['setting_config']['qq_appcode'],ENT_QUOTES); ?><?php echo html_entity_decode($output['setting_config']['sina_appcode'],ENT_QUOTES); ?><?php echo html_entity_decode($output['setting_config']['share_qqzone_appcode'],ENT_QUOTES); ?><?php echo html_entity_decode($output['setting_config']['share_sinaweibo_appcode'],ENT_QUOTES); ?><style type="text/css">body { _behavior: url(<?php echo SHOP_TEMPLATES_URL;?>/css/csshover.htc)}</style><link href="<?php echo SHOP_TEMPLATES_URL;?>/css/base.css" rel="stylesheet" type="text/css"><link href="<?php echo SHOP_TEMPLATES_URL;?>/css/home_group.css" rel="stylesheet" type="text/css"><link href="<?php echo SHOP_TEMPLATES_URL;?>/css/home_header.css" rel="stylesheet" type="text/css"><link href="<?php echo SHOP_RESOURCE_SITE_URL;?>/font/font-awesome/css/font-awesome.min.css" rel="stylesheet" />
<!--[if IE 7]>
...
<script src="<?php echo RESOURCE_SITE_URL;?>/js/dialog/dialog.js" id="dialog_js" charset="utf-8"></script><script type="text/javascript">
...
<div class="header-wrap"><header class="public-head-layout wrapper"><h1 class="site-logo"><a href="<?php echo SHOP_SITE_URL;?>"><img src="<?php echo UPLOAD_SITE_URL.DS.ATTACH_COMMON.DS.$output['setting_config']['site_logo']; ?>" class="pngFix"></a></h1><div class="title"><h2> <?php echo $lang['text_groupbuy'];?> </h2><?php if ($_GET['op'] == 'index' || strpos($_GET['op'], 'vr_groupbuy_') === 0) { ?><div class="city"> 地区[<a href="javascript:void(0)" id="button_show"><h3 id="show_area_name"><?php if (empty($output['city_name'])){ echo '全国';}else{ echo $output['city_name'];}?></h3><i class="arrow"></i></a>]</div><div id="list" class="list" style="display:none;"><a id="button_close" class="close" href="javascript:void(0)">&#215;</a><ul><li><a href="<?php echo urlShop('show_groupbuy', 'select_city'); ?>&back_op=<?php echo $_GET['op']; ?>&city_id=0"><?php echo $lang['text_country'];?></a></li><?php $names = $output['groupbuy_vr_cities']['name']; foreach ($output['groupbuy_vr_cities']['children'][0] as $v) { ?><li><a href="<?php echo urlShop('show_groupbuy', 'select_city'); ?>&back_op=<?php echo $_GET['op']; ?>&city_id=<?php echo $v; ?>"><?php echo $names[$v]; ?></a></li><?php } ?></ul></div><?php } ?></div><div class="head-search-layout"> <div class="head-search-bar" id="head-search-bar">
     <div class="hd_serach_tab" id="hdSearchTab">
      <ul><li act="search" class="current"><span>商品</span><i class="arrow"></i></li><li act="store_list"><span>店铺</span></li></ul>
<i></i>
</div>
...
<ul class="site-menu">
<li><a href="<?php echo urlShop('show_groupbuy', 'groupbuy_list'); ?>"<?php if (isset($output['groupbuyMenuIsVr']) && !$output['groupbuyMenuIsVr']) echo ' class="current"'; ?>>线上特卖</a></li>
<li><a href="<?php echo urlShop('show_groupbuy', 'vr_groupbuy_list'); ?>"<?php if (isset($output['groupbuyMenuIsVr']) && $output['groupbuyMenuIsVr']) echo ' class="current"'; ?>>线下特卖</a></li>
