...
<div class="tabmenu">
  <?php include template('layout/submenu');?>
    <a title="批量生成商品二维码" class="ncbtn ncbtn-aqua" href="index.php?act=store_goods_online&amp;op=maker_qrcode" style="right:100px" onclick="return confirm('您确定要执行批量生成二维码吗？');">批量生成商品二维码</a>
  ...
  <table class="search-form">
    ...
    <tr>
      ...
      <th> <select name="search_type">
          ...
          <option value="2" <?php if ($_GET['type'] == 2) {?>selected<?php }?>>SPU</option>
        </select>
      </th>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td class="nscs-table-handle"><?php if ($val['goods_lock'] == 0) {?>
        ...
        <?php } else {?>
        <span class="tip" title="该商品参加团购活动期间不能进行编辑及删除等操作,但可以编辑赠品"><a href="<?php if ($val['is_virtual'] ==1 ) {echo 'javascript:void(0);';} else {echo urlShop('store_goods_online', 'add_gift', array('commonid' => $val['goods_commonid']));}?>" class="btn-darkgray"><i class="icon-lock"></i>
        <p>锁定</p>
        </a></span>
        <?php }?></td>
    </tr>
