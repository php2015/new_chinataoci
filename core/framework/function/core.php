...
function getFlexigridArray($in_array, $fields_array, $data, $format_array)//shopwwi
{
	$out_array = $in_array;
...
function ncPriceFormatb($price)//shopwwi
{
...
function write_file($filepath, $data, $mode = null)
{
    ...
    $data = "<?php defined('ByShopWWI') or exit('Access Invalid!'); return ".$data.";";
 ...
 * @return stringurlAdminMobile
 */
function urlAdminMicroshop($act = '', $op = '', $args = array()){
...
function checkPlatformStoreBindingAllGoodsClass($store_id = null,$bind_all_gc = null){
	if($bind_all_gc==null){
		$bind_all_gc = $_SESSION['bind_all_gc']  ;
	}

	//      return checkPlatformStore($store_id) && $_SESSION['bind_all_gc'];
