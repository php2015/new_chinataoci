...
class Sms {
    ...
    public function send($mobile,$content) {
       $shopwwi_sms_type=C('shopwwi_sms_type');
		if($shopwwi_sms_type==1)
		{
			return $this->mysend_smsbao($mobile,$content);
		}
		if($shopwwi_sms_type==2)
		{
...
/*
	...
	*/
    private function mysend_smsbao($mobile,$content){
	   ...
	   $user_id = urlencode(C('shopwwi_sms_zh')); // 这里填写用户名
 	   $pass = urlencode(C('shopwwi_sms_pw')); // 这里填登陆密码
