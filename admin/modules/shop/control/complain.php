...
class complainControl extends SystemControl {
    ...
    private $links = array(
            array('url'=>'act=complain&op=complain_new_list','text'=>'新投诉'),
            array('url'=>'act=complain&op=complain_appeal_list','text'=>'待申诉'),
            array('url'=>'act=complain&op=complain_talk_list','text'=>'对话中'),
            array('url'=>'act=complain&op=complain_handle_list','text'=>'待仲裁'),
            array('url'=>'act=complain&op=complain_finish_list','text'=>'已关闭'),
            array('url'=>'act=complain&op=complain_subject_list','text'=>'主题设置'),
            array('url'=>'act=complain&op=complain_setting','text'=>'时效设置')
    );
    public function __construct() {
        parent::__construct();
        Language::read('complain');
        if ($_GET['op'] == 'index') $_GET['op'] = 'complain_new_list';
    ...
    */
    public function get_new_xmlOp() {
        ...
        foreach ($complain_list as $k => $complain_info) {
            $list = array();
            if ($_GET['state'] == 99) {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?act=complain&op=complain_progress&complain_id={$complain_info['complain_id']}\"><i class=\"fa fa-list-alt\"></i>查看</a>";
            } else {
                $list['operation'] = "<a class=\"btn orange\" href=\"index.php?act=complain&op=complain_progress&complain_id={$complain_info['complain_id']}\"><i class=\"fa fa-gavel\"></i>处理</a>";
            }
