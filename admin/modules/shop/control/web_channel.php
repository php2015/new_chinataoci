...
class web_channelControl extends SystemControl{
    private $links = array(
        array('url'=>'act=web_channel&op=web_channel','text'=>'频道列表'),
        array('url'=>'act=web_channel&op=floor_list','text'=>'模块列表')
    );
    ...
    public function get_channel_xmlOp() {
        ...
        $data = array();
        $data['now_page'] = $model_channel->shownowpage();
        $data['total_num'] = $model_channel->gettotalnum();
        $data['list'] = $out_list;
        echo Tpl::flexigridfXML($data);exit();
    }
