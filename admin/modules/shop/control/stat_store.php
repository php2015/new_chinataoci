...
class stat_storeControl extends SystemControl{
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (in_array($_REQUEST['op'],array('hotrank','storesales','get_storesales_xml','storesales_list','storesales_trends'))){
    ...
    public function newstoreOp(){
        ...
        if($_REQUEST['search_type'] == 'day'){
            ...
            if (C('dbdriver') == 'mysql') {
                ...
                $_group = 'DAY(FROM_UNIXTIME(store_time)),HOUR(FROM_UNIXTIME(store_time))';
            }
            $memberlist = $model->getNewStoreStatList($where, $field, 0, '', 0, $_group);
            if($memberlist){
        ...
        if($_REQUEST['search_type'] == 'week'){
            ...
            if (C('dbdriver') == 'mysql') {
                ...
                $_group = 'WEEKOFYEAR(FROM_UNIXTIME(store_time)),WEEKDAY(FROM_UNIXTIME(store_time))+1';
            }
            $memberlist = $model->getNewStoreStatList($where, $field, 0, '', 0, $_group);
            if($memberlist){
                foreach($memberlist as $k => $v){
        ...
        if($_REQUEST['search_type'] == 'month'){
            ...
            if (C('dbdriver') == 'mysql') {
                ...
                $_group = 'MONTH(FROM_UNIXTIME(store_time)),day(FROM_UNIXTIME(store_time))';
            }
            $memberlist = $model->getNewStoreStatList($where, $field, 0, '', 0, $_group);
            if($memberlist){
                foreach($memberlist as $k => $v){
    ...
    public function hotrank_listOp(){
        ...
        //查询统计数据
        $field = ' store_id,min(store_name) as store_name ';
        switch ($_GET['type']){
           case 'ordernum':
    ...
    public function showstoreOp(){
        ...
        if (!empty($_GET['id']) && is_array($_GET['id'])){
            $where['store_id'] = array('in',$_GET['id']);
        }
        ...
        if ($_GET['exporttype'] == 'excel'){
            ...
            $excel_data[0][] = array('styleid'=>'s_title','data'=>'店主商家账号');
            ...
            foreach ($store_list as $k => $v){
                ...
                if (empty($search_grade_list[$v['grade_id']])) {
                    $excel_data[$k+1][] = array('data'=>'平台店铺');
                } else {
    ...
    public function get_store_xmlOp(){
        ...
        $list = $model->getNewStoreStatList($where, '', $page);
        $statlist = array();
        if (!empty($list) && is_array($list)){
            $fields_array = array('store_name','member_name','seller_name','grade_id','store_end_time','store_time');
            foreach ($list as $k => $v){
                $out_array = getFlexigridArray(array(),$fields_array,$v);
                if (!empty($search_grade_list[$v['grade_id']])) {
                    $out_array['grade_id'] = $search_grade_list[$v['grade_id']];
                } else {
                    $out_array['grade_id'] = '平台店铺';
                }
                if ($v['store_end_time'] > 0) {
                    $out_array['store_end_time'] = date('Y-m-d',$v['store_end_time']);
                } else {
                    $out_array['store_end_time'] = '无限制';
                }
                $out_array['store_time'] = date('Y-m-d',$v['store_time']);
                $statlist[$v['store_id']] = $out_array;
            }
        }
        $data = array();
    ...
    public function storesales_listOp(){
        ...
        //列表字段
        $field = " store_id,min(store_name) as store_name,SUM(order_amount) as orderamount, COUNT(*) as ordernum, COUNT(DISTINCT buyer_id) as membernum";
        $orderby = 'membernum desc,store_id asc';
     ...
     * 输出店铺销售统计XML数据
     */
    public function get_storesales_xmlOp(){
        $model = Model('stat');
        //获得搜索的开始时间和结束时间
        $searchtime_arr = $model->getStarttimeAndEndtime($this->search_arr);
        $where = array();
        ...
        $field = " store_id,min(store_name) as store_name,SUM(order_amount) as orderamount, COUNT(*) as ordernum, COUNT(DISTINCT buyer_id) as membernum";
        //排序
        $order_type = array('store_name','membernum','ordernum','orderamount');
        ...
        $orderby = $sortname.' '.$sortorder.',store_id asc';
        $page = intval($_POST['rp']);
        if ($page < 1) {
            $page = 15;
        }
        $list = $model->statByStatorder($where, $field, array($page,$countnum), 0, $orderby, 'store_id');
        $statlist = array();
        if (!empty($list) && is_array($list)){
            foreach ($list as $k => $v){
                $out_array = getFlexigridArray(array(),$order_type,$v);
                $out_array['operation'] = "<a class='btn green' href='javascript:void(0);' nc_type='showtrends' data-param='{\"storeid\":\"{$v['store_id']}\"}'><i class='fa fa-line-chart'></i>走势图</a>";
                $out_array['orderamount'] = ncPriceFormat($v['orderamount']);
                $statlist[$v['store_id']] = $out_array;
            }
        }
    ...
    public function storesales_trendsOp(){
        ...
        $field = " min(store_id) as store_id,min(store_name) as store_name,SUM(order_amount) as orderamount, COUNT(*) as ordernum, COUNT(DISTINCT buyer_id) as membernum";
        ...
        //查询数据
        $statlist_tmp = $model->statByStatorder($where, $field, 0, '', 'timeval',$_group);
        //整理统计数组
        $storename = '';
        if($statlist_tmp){
