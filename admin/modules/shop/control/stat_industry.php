...
class stat_industryControl extends SystemControl{
    ...
    public function __construct(){
        ...
        //处理搜索时间
        if (!isset($this->search_arr['op']) || in_array($this->search_arr['op'],array('scale','rank','price'))){
        ...
        if (!isset($this->search_arr['op']) || in_array($this->search_arr['op'],array('scale','general'))){//仅显示前两级分类
            $show_depth = 2;
        }
            ...
            foreach ((array)$pricerange_arr as $k => $v){
                ...
                if (C('dbdriver') == 'mysql') {
                    ...
                    if ($v['e']){
                        ...
                        $field .= " ,SUM((case when goods_pay_price/goods_num > {$v['s']} and goods_pay_price/goods_num <= {$v['e']} then goods_pay_price else 0 end)) as orderamount_{$k}";
                    } else {//由于最后一个区间没有结束值，所以需要单独构造sql
                        $field .= " ,SUM((case when goods_pay_price/goods_num > {$v['s']} then goods_num else 0 end)) as goodsnum_{$k}";
                        $field .= " ,SUM((case when goods_pay_price/goods_num > {$v['s']} then goods_pay_price else 0 end)) as orderamount_{$k}";
                    }
                }
            }
     ...
     * 输出概况总览XML数据
     */
    public function get_general_xmlOp(){
        //获得子分类ID
        $gc_childid = $gc_childarr = array();
        if ($this->choose_gcid > 0){//如果进行了分类搜索，则统计该分类下的子分类
            $gc_childdepth = $this->gc_arr[$this->choose_gcid]['depth'] + 1;
        ...
        if ($gc_childarr){
            ...
            foreach ($gc_childarr as $k => $v){
                ...
                $out_array['priceavg'] = ncPriceFormat(($t = $goods_list[$v['gc_id']]['priceavg'])?$t:0);
                ...
                $out_array['orderamount'] = ncPriceFormat(($t = $ordergoods_list[$v['gc_id']]['orderamount'])?$t:0);
